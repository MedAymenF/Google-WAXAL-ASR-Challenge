...
def main():
    ...
    if training_args.do_train:
        if os.path.isdir(data_args.dataset_name):
            raw_datasets["train"] = load_from_disk(data_args.dataset_name)[data_args.train_split_name]
        else:
    ...
    if training_args.do_eval:
        if os.path.isdir(data_args.dataset_name):
            raw_datasets["eval"] = load_from_disk(data_args.dataset_name)[data_args.eval_split_name]
        else:
    ...
    # 4. Now we can instantiate the feature extractor, tokenizer and model
    ...
    if training_args.do_train:
        if training_args.resume_from_checkpoint is None:
            logger.info("Fine-tuning model from scratch")
            checkpoint = None
        else:
